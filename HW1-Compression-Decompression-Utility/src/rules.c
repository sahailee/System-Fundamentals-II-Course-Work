...
SYMBOL *new_rule(int v) {
    if(v >= FIRST_NONTERMINAL) {
        SYMBOL* rl = new_symbol(v, NULL);
        rl->prev = rl;
        rl->next = rl;
        rl->rule = rl;
        //ref_rule(rl);
        debug("New rule %s%lu%s for %d",
            IS_RULE_HEAD(rl) ? "[" : "<", SYMBOL_INDEX(rl), IS_RULE_HEAD(rl) ? "]" : ">", v);
        return rl;
    }
    //fprintf(stderr, "%s\n", "RETURNING NULL FROM NEW RULE");
    return NULL;
}
...
void add_rule(SYMBOL *rule) {
    ...
    debug("Add rule %s%lu%s",
            IS_RULE_HEAD(rule) ? "[" : "<", SYMBOL_INDEX(rule), IS_RULE_HEAD(rule) ? "]" : ">");
}
...
SYMBOL *ref_rule(SYMBOL *rule) {
    debug("Reference rule %s%lu%s for %d %d -> %d",
            IS_RULE_HEAD(rule) ? "[" : "<", SYMBOL_INDEX(rule), IS_RULE_HEAD(rule) ? "]" : ">", rule->value,
...
void unref_rule(SYMBOL *rule) {
    if((rule->refcnt) - 1 < 0) {
        fprintf( stderr, "ERROR: Reference count would become negative.\n");
        abort();
    }
    else{
         debug("Uneference rule %s%lu%s for %d %d -> %d",
            IS_RULE_HEAD(rule) ? "[" : "<", SYMBOL_INDEX(rule), IS_RULE_HEAD(rule) ? "]" : ">", rule->value,
            rule->refcnt, (rule->refcnt - 1));
        (rule->refcnt)--;
    }
