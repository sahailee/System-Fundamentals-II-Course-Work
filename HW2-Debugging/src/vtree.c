...
#define K(x)		(((x * 512) + 1023)/1024)	/* convert stat(2) blocks into
...
/* Define all our function prototypes that we need here. */
...
int		indent = 0,		/* current indent */
		...
		duplicates = FALSE,		/* use duplicate inodes */
...
/* open subdirectory */
	...
	for (file = readdir(dp); file != NULL; file = readdir(dp)) {
		if ((!quick && !visual ) ||
			...
			//debug("Size of rd_list: %ld", sizeof(struct RD_list *));
			//debug("Size of tmp_entry: %ld", sizeof(tmp_entry));
			tmp_RD->bptr = NULL;
			tmp_RD->fptr = NULL;
			if (head == NULL) {
				head = tmp_RD;
		...
		while (tmp_RD) {
			...
			while (tmp1_RD) {
				if (strcmp(NAME(tmp_RD->entry), NAME(tmp1_RD->entry)) > 0) {
					/* swap the two */
					memcpy(&tmp_entry, &tmp_RD->entry, sizeof(tmp_RD->entry));
					memcpy(&tmp_RD->entry, &tmp1_RD->entry, sizeof(tmp1_RD->entry));
	...
	if ( (!quick) && (!visual) ) {
		...
		for (file = readdir(dp); file != NULL; file = readdir(dp)) {
			...
			//debug("Value of Size: %ld", sizes);
		}
		//debug("Value of total sizes: %ld", total_sizes);
		//debug("Depth: %d", cur_depth);
		if (cur_depth<depth) {
...
#endif
	free(dp);	//Added this line to eliminate the memory leaks
...
# ifdef LINUX
	static struct option long_options[] = {
                   ...
                   {"no-follow-symlinks",    required_argument, 0,  'l' },
               };
    int option_index = 0;
...
# endif
		switch (option) {
			...
			case 'h':	depth = atoi(optarg);
					...
					break;
			case 'd':	duplicates = TRUE;
					break;
			case 'i':	cnt_inodes = TRUE;
					break;
		#ifdef MEMORY_BASED
			case 'o':	sort = TRUE; break;			//Should only appear for MEMORY_BASED
		#endif
			case 's':	sum = TRUE;
					break;
	...
	if (version > 0 ) {
		...
		if (version>1) {
			...
			if (duplicates) printf("Include duplicate inodes\n");
